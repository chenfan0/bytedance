...
- 快排 原理和优化 分析复杂度？
  - 选择一个元素，用两个指针，找到左边比他大的，以及右边比他小的，然后交换，直到左指针小于等于右指针。
  - 然后递归处理左边和右边的元素。
...
- 在字符串里面找到第 k 个唯一出现一次的字符串？
  - 使用map记录每个字符串出现的次数，然后在遍历获取第k个出现一次的即可。
...
- 说代码结果及原因：
        ...
        [1]==[1] // false 这里会进行地址值的比较
        [1]===[1] // false 这里会进行地址值的比较
        0.1+0.2==0.3  // false  0.1 + -0.2 == 0.3000000000000004  所以不相等 
- 二叉树先序中序 生成后序 ？
- 二叉树的最大深度？
- 2 维数组斜 45 度输出？
